...
pub async fn post_new_course_db(pool: &PgPool, new_course: Course) -> Result<Course, MyError> {
  let row = sqlx::query_as!(
    ...
    RETURING id, teacher_id, name, time, descritption, format, structure, duration, price, language, level"#,
...
pub async fn update_course_details_db(
  ...
  let course_row = sqlx::query_as!(
    Course,
    r#"UPDATE course SET
    name = $1, description = $2, format = $3, stucture = $4, duration = $5, price = $6, language = $7, level= $8
    WHERE teacher id = $9 and id = $10
    RETURING id, teacher_id, name, time, descritption, format, structure, duration, price, language, level"#,
    name,
    descritption,
    format,
